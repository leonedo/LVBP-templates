...
var equipos = {
    "tigres" : ["tigresinning9hc","fondotigreshc","tigresinning9vis","fondotigresvis"],
    "aguilas":["aguilasinning9vishc","fondoaguilashc","aguilasinning9vis","fondoaguilasvis"],
    "magallanes":["magallanesinnning9hc","fondomagallaneshc","magallanesinnning9vis","fondomagallanesvis"],
    "leones":["leonesinning9vishc","fondoleoneshc","leonesinning9vis","fondoleonesvis"],
    "tiburones":["tiburonesinning9hc","fondotiburoneshc","tiburonesinning9vis","fondotiburonesvis"],
    "cardenales":["cardenalesinnning9hc","fondocardenaleshc","cardenalesinnning9vis","fondocardenalesvis"],
    "caribes":["caribesinning9vishc","fondocaribeshc","caribesinning9vis","fondocaribesvis"],
    "bravos":["bravosinning9hc","fondobravoshc","bravosinning9vis","fondobravosvis"]
}
...
webcg.on('data', function (data) {
    ...
    for (key in data) {
       ...
       // if (key.includes("equipo")){equipo = data[key]}
        if (key.includes("equipos")){clear_logos(data[key])}
    ...
    animPromise.then(resolve => {
            ...
            setTimeout(() => {
                for (let i = 0; i < animElementsLength; i++) {
                    ...
                    if (animElement.data.hasOwnProperty('fondo') && animElement.data.fondo.includes('true')){
                        console.log(animElement.data.shapes[1].g.k.k)  
                    } 
...
function mover_fondo_visitante(campo){
    var fill = `.${campo}`
    document.querySelector(fill).setAttribute("transform","matrix(0.8499987125396729,0,0,0.8500000238418579,289.2498779296875,883.2490234375)")
}
...
function clear_logos(teamNameToSkip){
  console.log(teamNameToSkip)
  console.log(teamNameToSkip[0]) // --> visitante
  console.log(teamNameToSkip[1]) // --> homeclub
    for (var team in equipos) {
       if (team !== teamNameToSkip[0]) {
         campos = equipos[team]
         checkandupdate(campos[2],0);
         checkandupdate(campos[3],0);
        }
    }
    for (var team in equipos) {
        if (team !== teamNameToSkip[1]) {
          campos = equipos[team]
          checkandupdate(campos[0],0);
          checkandupdate(campos[1],0);
         }
     }
...
function update_equipos(nombre_equipo,homevisit){
    ...
    var campos = equipos[nombre_equipo]
    if (homevisit === 'homeclub'){
        checkandupdate(campos[0],1);
        checkandupdate(campos[1],1);
    }else{
        checkandupdate(campos[2],1);
        checkandupdate(campos[3],1);
    }
}
